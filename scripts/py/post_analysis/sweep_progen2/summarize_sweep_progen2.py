        ...
        for method_base in METHOD_ORDER:
            ...
            if forced_layer is not None:
                ...
                for method_name, meta in method_meta.items():
                    ...
                    if base != method_base:
                        continue
                    suffix = method_name[len(base) :]
                    try:
                        layer_idx = int(suffix)
                    except ValueError:
                        layer_idx = None
                    if layer_idx == forced_layer:
                        forced_name = method_name
                        break
                if forced_name is None:
                    continue
                best[dataset][method_base] = {
            ...
            for method_name, meta in method_meta.items():
                ...
                if base != method_base:
                    continue
                cond_stats: dict[str, dict[str, tuple[float, float]]] = {}
                for condition in CONDITIONS:
                    ...
                    if stats:
                        cond_stats[condition] = stats
                if not cond_stats:
                    continue
                scores = [
                    harmonic_mean(cond_stats.get(condition))
                    for condition in CONDITIONS
                    if condition in cond_stats
                ]
                valid_scores = [s for s in scores if math.isfinite(s)]
                if not valid_scores:
                    continue
                avg_score = statistics.fmean(valid_scores)
...
def build_rows(
    condition: str,
    aggregated: dict[str, dict[str, dict[str, dict[str, tuple[float, float]]]]],
    best_map: dict[str, dict[str, dict[str, str]]],
) -> list[list[str]]:
    rows: list[list[str]] = []
    for dataset in DATASET_ORDER:
        dataset_label = DATASET_LABELS.get(dataset, dataset)
        for method_base in METHOD_ORDER:
            best_info = best_map.get(dataset, {}).get(method_base)
            if not best_info:
                continue
