#!/usr/bin/env python3
"""
Enumerate decoding/steering sweep experiments for ProtGPT2.

Usage:
  python scripts/py/run/sweeps/sweep_protgpt2.py            # print all specs
  python scripts/py/run/sweeps/sweep_protgpt2.py --index 0  # single spec
  python scripts/py/run/sweeps/sweep_protgpt2.py --json     # JSON dump
"""
...
# ProtGPT2 uses 36 transformer layers.
PROTGPT2_NUM_LAYERS = 36
...
def build_methods() -> list[tuple[str, list[str]]]:
    ...
    # UCCS (contrastive layer) sweep over all layers
    for layer in range(PROTGPT2_NUM_LAYERS):
        methods.append(
            (
    ...
    # Probe steering sweep over all layers
    for layer in range(PROTGPT2_NUM_LAYERS):
        methods.append(
            (
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Enumerate ProtGPT2 decoding/steering sweep experiments.")
