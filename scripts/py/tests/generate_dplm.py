...
from replm.models.dplm.dplm import DiffusionProteinLanguageModel
...
def format_check(args):
    ...
    assert len(seq_list) == len(cond_position), "The length of cond_seq and cond_position does not match."
    position_list = []
    for pos in cond_position:
        pos = pos.split("-")
        assert len(pos) == 2, "The format of position is illegal, which is not correctly splited by '-'"
