...
def sparse_add(
    layer: int, dims: torch.Tensor, values: torch.Tensor, token_mask: torch.Tensor | None = None
) -> AffineEdit:
...
def sparse_mul(
    layer: int, dims: torch.Tensor, scales: torch.Tensor, token_mask: torch.Tensor | None = None
) -> AffineEdit:
    dims = dims.to(torch.long).reshape(-1)
